...
function NFTs() {
  ...
  const [profilePicNft, setProfilePicNft] = useState("");
  const { verifyMetadata } = useVerifyMetadata();

  useEffect(() => {
    if (isAuthenticated) {
      const currentPic = user?.get("profilePic");
      setProfilePicNft(currentPic);
    }
  }, [isAuthenticated, profilePicNft, user]);

  const handleClick = (nft) => {
    console.log("selected", nft);
    user
      .set("profilePic", nft.token_address)
      .save()
      .then(() => {
        setProfilePicNft(nft);
        console.log("SAVED", profilePicNft, nft);
      });
  };
